...
export default function VSBattle({ groupA, groupB, onChoose, round }) {
  return (
    <div className="relative flex flex-col items-center w-full max-w-5xl mx-auto px-4">
      ...
      <div className="flex flex-col md:flex-row gap-4 md:gap-6 w-full items-stretch">
        {/* Group A */}
        <GroupCard group={groupA} side="left" onClick={() => onChoose(groupA.id, groupB.id)} />

        {/* VS badge */}
        <div className="flex items-center justify-center md:self-center shrink-0 z-20">
          <div className="relative">
            <div className="w-16 h-16 md:w-20 md:h-20 rounded-full bg-gradient-to-br from-yellow-400 via-pink-500 to-purple-600
            ...
            {/* Glow ring */}
            <div className="absolute -inset-2 rounded-full bg-gradient-to-br from-yellow-400/20 via-pink-500/20 to-purple-600/20
              blur-md -z-10" />
          </div>
        </div>

        {/* Group B */}
        <GroupCard group={groupB} side="right" onClick={() => onChoose(groupB.id, groupA.id)} />
      </div>

      {/* Instruction */}
      <p className="mt-6 text-white/30 text-sm text-center">
        Choose the group you prefer
      </p>
    </div>
  );
}
